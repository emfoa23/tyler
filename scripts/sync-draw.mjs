...
// 3) 최신 회차 지연 필드 재보정 — 당첨금·판매액은 추첨 후 ~20:49, 1등 구매유형은 ~21:00 에야 공개되고
// 공개 전엔 null 이 아니라 0 으로 온다(2026-09-05 실측). 신규 루프는 넣은 회차를 다시 안 긁으므로,
// 최신 회차에 미공개 묶음이 남아 있으면 재조회하고 실제로 더 공개됐을 때만 upsert 한다.
// 판정은 lib/draw-state.mjs(화면과 공유) — 0 을 null 로 바꾸지 않고 원본 값으로 판단한다.
const head = (await select(
  "draws?select=draw_no,r1_winners,r5_winners,sales_total,first_auto,first_manual,first_semi&order=draw_no.desc&limit=1",
))[0];
if (head && needsLateFields(head)) {
