...
export function GenerateClient() {
  ...
  useEffect(() => {
    ...
    setClientId(cid);
    load(cid).catch(() => setError("기록을 불러오지 못했습니다."));
  }, [load]);
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl border border-stone-200 bg-white p-6 text-center">
        ...
        {data?.nextTarget && (
          <p className="mt-1 text-sm text-stone-500">
            지금 생성하면 <b>제{data.nextTarget}회</b>
            {data.nextTargetDate && ` (${dateShort(data.nextTargetDate)} 추첨)`} 대상으로 기록됩니다
          </p>
        )}
      ...
      <section className="space-y-3">
        <h2 className="font-bold">내 생성 기록</h2>
        <p className="text-xs text-stone-400">
          이 기기(브라우저) 기준으로 저장됩니다. 최근 50세트까지 표시.
        </p>
        {groups.length === 0 && (
